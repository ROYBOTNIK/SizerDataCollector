...
namespace SizerDataCollector.Core.Schema
{
	...
	public sealed class SchemaMap
	{
		...
		internal TableDefinition GetOrAddTable(string schema, string name)
		{
		...
		internal bool TryGetTable(string schema, string name, out TableDefinition table)
			=> _tables.TryGetValue(ToKey(schema, name), out table);

		internal IEnumerable<TableDefinition> GetTables() => _tables.Values;

		internal static string ToKey(string schema, string name)
	...
	public sealed class TableDefinition
	{
		...
		internal void AddPrimaryKey(IEnumerable<string> columns)
		{
		...
		internal void AddReference(string tableName) => _references.Add(tableName);
		internal void AddReferencedBy(string tableName) => _referencedBy.Add(tableName);
	}
