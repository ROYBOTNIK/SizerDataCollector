-- Throughput CAGGs using direct join to machine_settings (no table-reading functions inside CAGGs)
-- Drops and recreates throughput CAGGs and view; reapplies policies.

-- Drop dependents in safe order (view -> daily -> minute) to avoid dependency errors
...
-- Minute-level (oee) with machine_settings join for recycle_outlet/target_throughput
...
-- Minute-level (public) per-serial with machine_settings join
...
-- Recreate view with target_throughput column from the CAGG
...
SELECT minute_ts,
       ...
       oee.calc_perf_ratio(total_fpm, missed_fpm, combined_recycle_fpm, target_throughput) AS throughput_ratio
FROM oee.cagg_throughput_minute_batch;
